...
import EnforceSopOperatorPanel from "@/components/session/EnforceSopOperatorPanel";
import { type FrictionReport, parseFrictionReport } from "@/lib/frictionReport";
      ...
      if (srv.enforce_focus && srv.challenge_cycle >= 0) {
        if (srv.challenge_status === "pass") {
          ...
          if (challengeResultNotifiedCycle.current !== srv.challenge_cycle) {
            ...
            if (!challengeSilencedRef.current) setShowSuccessAnim(true);
  ...
  return (
    <>
      ...
      {showSuccessAnim && createPortal(
        <XtnlChallengePassedAnimation onDone={() => setShowSuccessAnim(false)} />,
        document.body
      )}
...
export default function SessionClient({ user, viewMode, roles }: { user: User; viewMode: "operator" | "analyst"; roles: string[] }) {
  ...
  const [opSidebarTool,  setOpSidebarTool]  = useState<"checklist" | "comment" | "enforce">("comment");

  /* If a strategist disables the checklist while it's the active tab,
     fall back to Add Comment rather than leaving an empty pane selected. */
  useEffect(() => {
    if (!showChecklist && opSidebarTool === "checklist") setOpSidebarTool("comment");
  }, [showChecklist, opSidebarTool]);
