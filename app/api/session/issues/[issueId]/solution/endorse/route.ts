...
/** POST — increment endorsement count on the active solution for an issue.
...
export async function POST(
  ...
  const { data: sol, error: fetchErr } = await supabase
    ...
    .select("solution_id, endorsements")
    .eq("issue_id", issueId)
  ...
  const { error: updateErr } = await supabase
    .from("issue_solutions")
    .update({ endorsements: (sol.endorsements ?? 0) + 1 })
    .eq("solution_id", sol.solution_id);

  if (updateErr)
    return NextResponse.json({ error: updateErr.message }, { status: 500 });

  return NextResponse.json({ ok: true, endorsements: (sol.endorsements ?? 0) + 1 });
}
